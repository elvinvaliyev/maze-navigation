...
class PerformanceAnalyzer:
    ...
    def calculate_efficiency_metrics(self) -> Dict:
        ...
        for agent in self.results['agent'].unique():
            ...
            # Learning efficiency (improvement per episode)
            avg_learning_efficiency = np.mean(agent_data['learning_improvement'] / 100)  # per episode
    ...
    def learning_curve_analysis(self) -> Dict:
        ...
        agent_learning = self.results.groupby('agent')['learning_improvement'].agg(['mean', 'std', 'min', 'max']).round(3)
        learning_analysis['agent_learning'] = agent_learning
        
        # Learning improvement by environment
        maze_learning = self.results.groupby('maze')['learning_improvement'].mean().sort_values(ascending=False)
        learning_analysis['maze_learning'] = maze_learning
        
        # Learning improvement by reward configuration
        reward_learning = self.results.groupby('reward_config')['learning_improvement'].mean().sort_values(ascending=False)
